...
def test_order_lifecycle_track_replace_cancel() -> None:
    ...
    order = Order(order_id="1", trading_pair="BTC/USD", side="buy", price=100.0, amount=0.5)

    manager.track(order)
    assert manager.get_open_order("1") == order

    replacement = Order(order_id="1", trading_pair="BTC/USD", side="buy", price=101.0, amount=0.4)
...
def test_order_manager_rejects_duplicate_and_handles_missing() -> None:
    manager = OrderManager()
    order = Order(order_id="2", trading_pair="ETH/USD", side="sell", price=200.0, amount=1.0)
